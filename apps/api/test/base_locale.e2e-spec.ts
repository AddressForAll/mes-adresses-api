...
describe('BASE LOCAL MODULE', () => {
  ...
  describe('POST :baseLocaleId/upload - codeVoie', () => {
    it('Upload CSV with codeVoie for voie - stores codeVoie in DB', async () => {
      const balId = await createBal({ nom: 'bal', commune: '08053' });

      const csvContent = `cle_interop;voie_nom;numero;commune_insee;position;long;lat;date_der_maj
08053_ABCD_00001;Rue de la Paix;1;08053;entree;5.123;48.456;2024-01-01`;
      ...
      expect(voies[0].codeVoie).toEqual('abcd');
    });

    it('Upload CSV with codeVoie for toponyme - stores codeVoie in DB', async () => {
      const balId = await createBal({ nom: 'bal', commune: '08053' });

      const csvContent = `cle_interop;voie_nom;numero;commune_insee;position;long;lat;date_der_maj
08053_TOPO_99999;Le Hameau;99999;08053;entree;5.123;48.456;2024-01-01`;
      ...
      const toponymes = await repositories.toponymes.find({ where: { balId } });
      expect(toponymes).toHaveLength(1);
      expect(toponymes[0].codeVoie).toEqual('topo');
    });

    it('Upload CSV with default codeVoie (xxxx) - stores null in DB', async () => {
      const balId = await createBal({ nom: 'bal', commune: '08053' });

      const csvContent = `cle_interop;voie_nom;numero;commune_insee;position;long;lat;date_der_maj
08053_xxxx_00001;Rue de la Paix;1;08053;entree;5.123;48.456;2024-01-01`;
